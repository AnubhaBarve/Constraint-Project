...
for reg in param['regions']:
    ...
    for year in range (param['styear'], param['endyear'] + 1):
        ...
        param['outputDir'] = 'S:\\asset ops\\GO_Group\\Interns\\2019\\Anubha\\Constraint Project\\Constraint-Project\\Data\\Constraint Data\\'

        outputFile = reg + '_HourlyConstraintData_' + \
        str(year) + '-01-01' + '_to_' + \
        enDate.strftime('%Y-%m-%d') + '.xlsx'
        workbook = xlsxwriter.Workbook(param['outputDir'] + outputFile)

        ## URL address for the data
        address = 'https://services.yesenergy.com/PS/rest/constraint/hourly/DA/' + reg + \
