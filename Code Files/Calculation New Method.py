...
def calculationLODF(constraints, outages):
    ...
    constraints = constraints.loc[(constraints['date'] >= date_range.at[0,'strtdate']) & (constraints['date'] <= date_range.at[0,'enddate'])]
    ...
    for i in constraints_unique['date']:
        ...
        outages_sample = outages.loc[(outages['start'] >= i) & (outages['end'] <= i)]
        if not outages_sample.empty:
            print(constraint_sample)
            print(outages_sample)
            openall_outage(outages_sample)
    ...
    writer = pd.ExcelWriter(r"S:\asset ops\GO_Group\Interns\2019\Anubha\Constraint Project\Constraint-Project\Data\Trial Data\CalculationLODFNew.xlsx")

    # writing to the created excel file and storing the sheet as "LODF"
    finalLODF.to_excel(writer, "LODF")
...
def openall_outage(outages):
    ...
    for it, ro in outages.iterrows():
        close_open_branches = np.append(close_open_branches,
                                        ro[['from_bus_number', 'to_bus_number', 'circuit_id']])
        close_open_branches = np.append(close_open_branches, 'Open')  # LineStatus
    # Change the parameters using the newly closed branches
    simauto.ChangeParametersMultipleElementFlatInput('branch',
                                                         ['from_bus_number', 'to_bus_number', 'circuit_id', 'open_close'],
                                                         len(outages), close_open_branches)

    filename = r"S:\asset ops\GO_Group\Interns\2019\Anubha\Constraint Project\Constraint-Project\Data\InterfaceDefinition_Updated.pwb"
...
def powerworldPTDF(outages, monitoredConstraints):
    ...
    writer = pd.ExcelWriter(
        r"S:\asset ops\GO_Group\Interns\2019\Anubha\Constraint Project\Constraint-Project\Data\Trial Data\CalculationPTDF.xlsx")
...
def main():
    ...
    calculationLODF(inputfile, file)
