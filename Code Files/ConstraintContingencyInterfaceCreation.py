...
# File name: ConstraintContingencyInterfaceCreation.py
# Purpose: The purpose of this file is, given a list of constraint-contingency pair it
# defines the pair in powerworld format.
...
# A function named InterfaceMap which stores the Mapped Contingency-Monitored Element
# in a PowerWorld Format in an excel file.


def InterfaceMap(uniquePairs):

    # creating a new DataFrame that matches PowerWorld Format
    powerWorldFormat = pd.DataFrame(columns=["Interface Name", "Element", "Meter Far", "Weight", "Contingency Name", "Monitored Element Name"])
    ...
    for i,r in tqdm(uniquePairs.iterrows()):
        ...
        # check if the Contingency Column for From Bus Number is empty and constraint is mapped - if yes, store the details in powerWorldFormat DataFrame (Base Case)
        if(r["contingency_from_bus_number"]) == " " and (r["constraint_from_bus_number"]) != " ":
            ...
            powerWorldFormat = powerWorldFormat.append({"Interface Name": a, "Element": b, "Meter Far": c, "Weight": d, "Contingency Name": f, "Monitored Element Name" : g}, ignore_index=True)

        # else store the details in powerWorldFormat DataFrame (Constraint-Contingency Pair)
        else:
            a = uniquePairs.at[i, "interface_name"]
            b = uniquePairs.at[i, "element_b_monitored"]
            c = uniquePairs.at[i, "meter_far"]
            ...
            f = uniquePairs.at[i, "contingency"]
            g = uniquePairs.at[i, "facilityname"]
            powerWorldFormat = powerWorldFormat.append({"Interface Name": a, "Element": b, "Meter Far": c, "Weight": d, "Monitored Element Name": g, "Contingency Name": f}, ignore_index=True)
            powerWorldFormat = powerWorldFormat.append({"Interface Name": a, "Element": e, "Meter Far": c, "Weight": d, "Monitored Element Name": g, "Contingency Name": f}, ignore_index=True)

    # Removing the duplicate Interface Names from the Mapped List
    powerWorldFormatUnique = powerWorldFormat.drop_duplicates(subset=["Interface Name","Element"], keep='first')
    # preserving the index of the dataframe
    powerWorldFormatUnique = powerWorldFormatUnique.reset_index(drop=True)

    # returning the resultant dataframe
    return powerWorldFormatUnique
...
def main():
    ...
    # The uniquePairs14 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2014
    ...
    # call to InterfaceMap function and concatenating all resulting dataframes into one'''
    ...
    # The uniquePairs15 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2015
    ...
    # The uniquePairs16 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2015
    ...
    # The uniquePairs17 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2015
    ...
    # The uniquePairs18 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2015
    ...
    # The uniquePairs19 variable reads the excel file that contains
    # information of unique constraint-contingency pair 2015
